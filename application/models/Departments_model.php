...
class Departments_model extends MY_Model
{
    ...
    protected $table = 'departments_buget';
    ...
    function get_all()
    {
        $sql = "SELECT * FROM departments_buget";
        return $this->db->query($sql)->result_array();
    }


    function option_departments()
    {
        $this->db->order_by('name', 'ASC');
        $option_departments = $this->db->get('ref_departments');
        return $option_departments->result_array();
    }

    function option_departments_detail($id_def)
    {
        $option_departments_detail = "<option value='0'>Option</pilih>";
        $this->db->order_by('name', 'ASC');
        $detail = $this->db->get_where('ref_departments_detail', array('id_def' => $id_def));
        foreach ($detail->result_array() as $data) {
            $option_departments_detail .= "<option value='$data[id_def]'>$data[name]</option>";
        }
        return $option_departments_detail;
    }
    ...
    function count_departments()
    {
        ...
        // if ($user == 1) {
            $sql = "SELECT COUNT(id) AS total FROM departments_buget";
            return $this->db->query($sql)->result_array();
        // } else {
