...
class Pelanggan_model extends MY_Model
{
    ...
    function count_pengirim()
    {
        $user = $this->session->userdata('id');
        if ($user == 1) {
            $sql = "SELECT COUNT(id) AS total FROM tabel_pelanggan";
            return $this->db->query($sql)->result_array();
        } else {
            $sql = "SELECT COUNT(id) AS total FROM tabel_pelanggan WHERE user_admin_dibuat='$user'";
            return $this->db->query($sql)->result_array();
        }
    }
}
