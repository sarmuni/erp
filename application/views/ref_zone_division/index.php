...
<div class="col-12">
    <div class="card mb-3">
        ...
        <div class="card-body">
            <a role="button" href="#" class="btn bg-danger btn-sm" title="Add" data-toggle="modal" data-target=".tambah_ref_zone_division">
            ...
            <a role="button" href="<?php base_url('ref_zone_division'); ?>" class="btn bg-danger btn-sm" title="Refresh">
            ...
            <div class="table-responsive">
                <table id="dataTable" class="table table-bordered table-hover display" style="width:100%">
                    ...
                    <?php
                    $no = 1;
                    foreach ($ref_zone_division as $row) {
                    ?>
                        <tr>
                            <td><?php echo $no; ?></td>
                            <td><?php echo strtoupper($row['name']); ?></td>
                            <td><?php echo strtoupper($row['gln_zd']); ?></td>
                            <td><?php echo strtoupper($row['name_factory']); ?></td>
                            <td><?php echo $row['description']; ?></td>
                            <td>
                                <a role="button" href="#" class="btn bg-warning btn-sm" title="Edit" data-toggle="modal" data-target="#edit_ref_zone_division<?php echo $row['id']; ?>"><i class="fas fa-user-edit"></i>
                                </a>
                                
                                 <a role="button" href="<?php echo site_url(); ?>ref_zone_division/delete/<?php echo $row['id']; ?>" id="deleted" class="btn bg-danger btn-sm tombol-hapus" title="delete record"><i class="fas fa-trash-alt"></i>
                                 </a>
...
<!-- Modal Add -->
<div class="modal fade tambah_ref_zone_division" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Form Add Master Zone Division</h5>
            ...
            <div class="modal-body">
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <form autocomplete="off" action="<?php echo base_url('ref_zone_division/add'); ?>" method="POST" enctype="multipart/form-data">
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="id_loc_fac">Factory Name</label>
                                    <select id="id_loc_fac" class="form-control form-control-sm" name="id_loc_fac" id="id_loc_fac" value="<?= set_value('id_loc_fac'); ?>">
                                        <option selected="">Select</option>
                                        <?php foreach ($ref_location_factory as $select) { ?>
                                            <option value="<?php echo $select['id'] ?>"><?php echo $select['name'] ?></option>
                                        <?php } ?>
                                    </select>
                                    <?= form_error('id_loc_fac', '<p style="color:red; font-size:12px;">', '</p>'); ?>
                                </div>
...
<!-- Modal Edit -->
<?php
foreach ($ref_zone_division as $i) :
    $id                         = $i['id'];
    $id_loc_fac                 = $i['id_loc_fac'];
    $gln_zd                     = $i['gln_zd'];
    $name                       = $i['name'];
    $description                = $i['description'];
?>
    <div class="modal fade edit_ref_zone_division" id="edit_ref_zone_division<?php echo $id; ?>" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Form Edit Zone Division</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <form autocomplete="off" action="<?php echo base_url('ref_zone_division/edit/' . $id); ?>" method="POST" enctype="multipart/form-data">
                                <div class="form-row">

                                <div class="form-group col-md-6">
                                        <label for="id_loc_fac">Factory Name</label>
                                        <select id="id_loc_fac" class="form-control form-control-sm" name="id_loc_fac" id="id_loc_fac">
                                            <option selected="">--Select--</option>
                                            <?php foreach ($ref_location_factory as $select) { ?>
                                                
                                                <?php if ($select['id']==$id_loc_fac) { ?>
